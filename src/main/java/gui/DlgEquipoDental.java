...
public class DlgEquipoDental extends JDialog implements ActionListener {
	...
	void listar() {
		...
		try {
			List<EquipoDental> lstEquipo = manager.createQuery(jpql, EquipoDental.class).getResultList();
			txtSalida.setText("");
			for (EquipoDental e : lstEquipo) {
				...
				imprimir("Dentista............: " + e.getDentista().getNombreCompleto());
				imprimir("--------------------------------------");
				imprimir();
			}
		} finally {
